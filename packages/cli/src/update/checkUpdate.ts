...
export async function checkUpdate(options?: { force?: boolean }): Promise<void> {
  try {
    await _checkUpdate(options?.force ?? false);
  } catch {
    // never surface update errors to the user
  }
}

async function _checkUpdate(force: boolean): Promise<void> {
  ...
  // If cache is fresh and not forced, use it — but only notify if we haven't shown this version before
  if (!force && cache && now - cache.storedAt < CACHE_TTL_MS) {
  ...
  // Stale, no cache, or forced — fetch fresh
  const latestVersion = await fetchLatestVersion();
  if (!latestVersion) {
    if (!force && cache) await writeCache({ ...cache, storedAt: now });
    return;
  }

  const latest = parseVersion(latestVersion);
  if (!latest || latest <= current) {
    if (!force) await writeCache({ latestVersion, storedAt: now, notifiedVersion: cache?.notifiedVersion });
    else console.error("Already up to date.");
    return;
  }
